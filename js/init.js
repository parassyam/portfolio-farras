...
------------------------------------------------------ */
    ...
    // Fungsi untuk mengacak ulang posisi elemen keterampilan
function shuffleSkillsPosition() {
   var skills = document.querySelectorAll('.skill-item');
   skills.forEach(function(skill) {
      var randomX = Math.random() * 60 - 30; // Nilai random untuk posisi horizontal
      var randomY = Math.random() * 60 - 30; // Nilai random untuk posisi vertikal
      skill.style.transform = `translate(${randomX}px, ${randomY}px)`;
   });
}

// Panggil fungsi shuffleSkillsPosition() saat halaman dimuat
window.addEventListener('load', shuffleSkillsPosition);

// Dapatkan elemen yang akan dianimasikan
const animatedElement = document.querySelector('.tracking-in-expand');
...
// Tambahkan event listener untuk memicu animasi saat halaman digulir
window.addEventListener('scroll', () => {
   const scrollPosition = window.scrollY + window.innerHeight;
   const elementPosition = animatedElement.getBoundingClientRect().top + window.scrollY;
